...
async def ingest_document(
    ...
    if file_path:
        ...
        raw_docs = load_from_path(file_path)
    elif source:
        ...
        raw_docs = load_from_bytes(file_bytes, filename)
    else:
    ...
    # Step 2: Chunk
    chunks = chunk_documents(raw_docs, strategy=chunker_strategy)
...
async def ingest_batch(
    ...
    """Ingest multiple documents. Each item should have 'metadata' and 'source'."""
    results = []
    for item in items:
...
async def delete_document_chunks(record_id: str) -> int:
    ...
    store = get_vector_store()
    # PGVector supports filtering deletes
