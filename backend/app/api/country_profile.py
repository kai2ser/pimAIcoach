...
def _export_docx(content: str, country_name: str) -> Response:
    ...
    # Title
    title = doc.add_heading(f"Public Investment Management Context: {country_name}", level=1)
    ...
    filename = f"PIM_Profile_{country_name.replace(' ', '_')}.docx"
    return Response(
        content=buf.read(),
...
def _export_pdf(content: str, country_name: str) -> Response:
    ...
    pdf.cell(0, 5, f"PIM Country Briefing — {country_name}", align="R", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)

    # Title
    pdf.set_font("Helvetica", "B", 16)
    pdf.set_text_color(29, 78, 216)
    pdf.cell(0, 10, f"Public Investment Management Context: {country_name}", new_x="LMARGIN", new_y="NEXT")
    ...
    filename = f"PIM_Profile_{country_name.replace(' ', '_')}.pdf"
    return Response(
        content=buf.read(),
